...
jQuery(document).ready(function($) {
	...
	// Products that has variation of size
	$('.sizeOf').change(function(){
		$('.variation_id').val($('option:selected',this).attr('data-variation'));
	});

	// Products that has Screen
	$('.screenType').change(function(){
		$('.attribute_name').val($('option:selected',this).attr('data-variation'));
	});
	...
	if( $map.length ) {
		$map.gMap({
			address: 'Av. Paulista, 1159 - Bela Vista - São Paulo - SP',
			zoom: 16,
			controls: false,
			scrollwheel: false,
			markers: [
